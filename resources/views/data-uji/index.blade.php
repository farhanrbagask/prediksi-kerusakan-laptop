...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            ...
            <p class="text-gray-400 text-sm mt-1">Kelola data testing untuk evaluasi sistem naive bayes</p>
        </div>
        <div class="flex flex-wrap gap-3 w-full sm:w-auto justify-end">
            <button onclick="confirmPrediksiTerakhir()"
                class="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-gradient-to-r from-teal-500 to-teal-600 hover:from-teal-600 hover:to-teal-700 text-white font-medium py-2.5 px-5 rounded-xl transition-all duration-300 hover:shadow-lg hover:shadow-teal-500/30 border border-teal-400/20">
            ...
            <form id="prediksi-terakhir-form" action="{{ route('data-uji.prediksi-terakhir') }}" method="GET"
                class="hidden"></form>

            <button onclick="confirmPrediksiAll()"
                class="flex-1 sm:flex-none flex items-center justify-center gap-2 bg-gradient-to-r from-purple-500 to-purple-600 hover:from-purple-600 hover:to-purple-700 text-white font-medium py-2.5 px-5 rounded-xl transition-all duration-300 hover:shadow-lg hover:shadow-purple-500/30 border border-purple-400/20">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />
                </svg>
                <span>Prediksi Semua</span>
            </button>

            <form id="prediksi-form" action="{{ route('prediksi.semua') }}" method="GET" class="hidden"></form>
            ...
            <form id="delete-all-form" action="{{ route('data-uji.truncate') }}" method="POST" class="hidden">
                @csrf
                @method('DELETE')
            </form>
                        ...
                        <tr class="hover:bg-white/10 transition-colors">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-center">
                                @if ($data->hasil_prediksi)
                                    <span
                                        class="px-3 py-1.5 text-xs font-bold rounded-full border-2 text-white
                                        @if ($data->hasil_prediksi == 'K1') bg-blue-500/50 border-blue-300
                                        @elseif($data->hasil_prediksi == 'K2') bg-green-500/50 border-green-300
                                        @elseif($data->hasil_prediksi == 'K3') bg-yellow-500/50 border-yellow-300
                                        @elseif($data->hasil_prediksi == 'K4') bg-purple-500/50 border-purple-300
                                        @elseif($data->hasil_prediksi == 'K5') bg-red-500/50 border-red-300
                                        @else bg-gray-500/50 border-gray-300 @endif">
                                        {{ $data->hasil_prediksi }}
                                    </span>
                                @else
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-center">
                                <div class="flex items-center justify-center gap-2">
                                    ...
                                    @if ($data->hasil_prediksi)
                                        <a href="{{ route('prediksi.perhitungan', $data->id_uji) }}"
                                            class="text-white hover:text-white transition-colors p-1.5 hover:bg-white/20 rounded-lg bg-white/10 border border-white/20"
                                    ...
                                    <button onclick="openEditModal('{{ $data->id_uji }}')"
                                        class="text-white hover:text-white transition-colors p-1.5 hover:bg-white/20 rounded-lg bg-white/10 border border-white/20"
                                        title="Edit">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                    ...
                                    <form action="{{ route('data-uji.destroy', $data->id_uji) }}" method="POST"
                                        class="inline">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="14" class="px-6 py-12 text-center">
                                <div class="bg-white/10 backdrop-blur-sm rounded-2xl p-8 border border-white/20">
                                    ...
                                    <p class="text-xl font-bold text-white mb-2">Belum ada data uji</p>
                                    <p class="text-white">Klik tombol Tambah Data untuk memulai</p>
                                </div>
                            </td>
                        </tr>
                    @endforelse
            ...
            <div
                ...
                <div class="px-6 py-5 bg-white">
                    ...
                    <div id="formTab" class="space-y-4">
                        <form id="dataForm" method="POST" action="{{ route('data-uji.store') }}">
                            @csrf
                    ...
                    <div id="excelTab" class="hidden space-y-4">
                        <form id="excelForm" method="POST" action="{{ route('data-uji.import') }}"
                            ...
                            <div
                                ...
                                <div class="flex justify-center gap-3">
                                    <a href="{{ route('data-uji.template') }}"
    ...
    <script>
        const editData = @json($dataUji->items());
        ...
        function openAddModal() {
            ...
            document.getElementById('dataForm').action = "{{ route('data-uji.store') }}";
        ...
        function openEditModal(id) {
            const data = editData.find(d => d.id_uji === id);
            if (!data) return;
            ...
            document.getElementById('dataForm').action = "{{ url('data-uji') }}/" + id;
        ...
        function confirmDeleteAll() {
            if (confirm('Yakin ingin menghapus SEMUA data uji? Tindakan ini tidak dapat dibatalkan!')) {
                document.getElementById('delete-all-form').submit();
            }
        }

        function confirmPrediksiAll() {
            if (confirm('Jalankan prediksi untuk semua data uji?')) {
                document.getElementById('prediksi-form').submit();
            }
        }

        function confirmPrediksiTerakhir() {
            if (confirm('Jalankan prediksi untuk semua data uji terakhir?')) {
                document.getElementById('prediksi-terakhir-form').submit();
            }
        }
